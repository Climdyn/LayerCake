...
class PlanarChannelFourierBasis(SymbolicBasis):
    ...
    direction :math:`y`. The coordinate :math:`y` extent is symmetric around zero.
    ...
    first_y_mode: str, optional
        `'sin'` to get the first meridional mode in :math:`\\sin y`,
        or `'cos'` to get it in :math:`\\cos y`.
    ...
    def __init__(self, parameters, spectral_blocks, length=None, first_y_mode='sin'):
        ...
        for wv in awavenum:
            ...
            mode_eq = fourier_functions(wv, self._n, self.coordinate_system, first_y_mode)
    ...
    def set_parameters(self, parameters):
        ...
        coordinate_system = PlanarCartesianCoordinateSystem(extent=((0., length), (-pi / 2, pi / 2)))
...
def contiguous_channel_basis(nxmax, nymax, parameters, length=None, first_y_mode='sin'):
    ...
    return PlanarChannelFourierBasis(parameters, spectral_blocks, length, first_y_mode)


def fourier_functions(wave_number, n, coordinate_system, first_y_mode):
    """Function that return Fourier modes expressions:

    * `'A'` for a function of the form :math:`F^A_{P} (x, y) =  \\sqrt{2}\\, \\sin(P y) = \\sqrt{2}\\, \\cos(n_y\\, y)`
    * `'K'` for a function of the form :math:`F^K_{M,P} (x, y) =  2\\cos(M nx)\\, \\cos(P y) = 2\\cos(n_x\\,  n\\, x)\\, \\cos(n_y\\, y)`
    * `'L'` for a function of the form :math:`F^L_{H,P} (x, y) = 2\\sin(H nx)\\, \\cos(P y) = 2\\sin(n_x\\, n \\,x)\\, \\cos(n_y\\, y)`
    ...
    if wave_number.type == 'A':
        if first_y_mode == 'sin':
            if wave_number.ny % 2:
                mode_eq = sqrt(2) * sin(wave_number.ny * y)
            else:
        ...
        if wave_number.ny % 2:
            mode_eq = 2 * cos(wave_number.nx * n * x) * cos(wave_number.ny * y)
        else:
            mode_eq = 2 * cos(wave_number.nx * n * x) * sin(wave_number.ny * y)
    elif wave_number.type == 'L':
        if wave_number.ny % 2:
            mode_eq = 2 * sin(wave_number.nx * n * x) * cos(wave_number.ny * y)
        else:
...
def channel_wavenumbers(spectral_blocks):
    ...
    for i in range(spectral_blocks.shape[0]):  # function type is limited to AKL for the moment: atmosphere is a channel
        ...
        if spectral_blocks[i, 0] == 1:
            ...
            # wave_numbers.append(WaveNumber('B', spectral_blocks[i, 1], 0, 0, 0, spectral_blocks[i, 1]))
