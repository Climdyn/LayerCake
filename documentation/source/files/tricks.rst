...
`Reinhold & Pierrehumber model <../../../examples/atmospheric/baroclinic_one_layer.py>`_ one.
...
Running the `LayerCake code <../../../examples/atmospheric/baroclinic_one_layer.py>`_  produce the following figure:
...
a :class:`~layercake.arithmetic.terms.linear.LinearTerm` term involving a :class:`~layercake.variables.fields.ParameterField` field representing
the spatial variation of the parameter, and the Laplacian terms representing :math:`\nabla^2 (\psi - \theta)`.
It is implemented with a few lines added to the `Reinhold & Pierrehumber model code <../../../examples/atmospheric/baroclinic_one_layer.py>`_:
...
The impact of this spatial variation of the bottom friction coefficient on the model's dynamic
